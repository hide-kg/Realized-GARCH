function [estimpara, forecast_fit, logL, stderror, tstat] ...
    = realized_garch_midas(daily_return, RV, period, logtau, type)
%
% 2021/5/18
%   Realized GARCH MIDASのプログラム
%
% input -
%   daily_return : 日次リターン
%   RV : 実現ボラティリティ
...
%   m, lambda, kappa : MIDAS式のパラメータ


% パラメータの初期値
%   [omega, beta, gamma, xi, phi, sigma_u, tau1, tau2, m, lambda, kappa]
para0 = [0.1, 0.3, 0.6, 0.1, 0.9, 0.4, 0.1, 0.1, 0.1, 0.5, 5];

% 月次のRVを作成する
% Y : freq-by-nMonth matrix form of observations (nobs = freq * nMonth)
% RV_mat : freq-by-nMonth matrix form of observations 
% RVm : 1-by-nMonth realized volatility (monthly)

nobs = numel(daily_return);
nMonth = ceil(nobs/period);
Y = NaN(period, nMonth);
Y(1:nobs) = daily_return(:);
RV_mat = NaN(period, nMonth);
RV_mat(1:nobs) = RV(:);
RVm = nansum(RV_mat,1);
if mod(nobs, period) > 0
    RVm(end) = nansum(RV_mat(end - period + 1:end));
end

llh = @(x0) -realized_garch_llh(x0, Y, RV, RVm, type);
...
[llh, llhs, h, z] = realized_garch_llh(para, Y, RV, RVm, type, ver);

fun = @(x0) realized_garch_llh(x0, Y, RV, RVm, type);
...
function [llh, llhs, h, z] = realized_garch_llh(para0, daily_return, RV, RVm, type)
% Realized GARCHの対数尤度関数の計算
%
% input -
%   para0 : 初期パラメータ
%   daily_reutrn : freq-by-nMonth matrix form of daily returns (nobs = freq * nMonth)
%   RV_mat : freq-by-nMonth matrix form of daily RV 
%   RVm :1-by-nMonth
