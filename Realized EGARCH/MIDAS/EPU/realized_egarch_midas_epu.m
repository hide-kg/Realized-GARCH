function [estimpara, forecast_fit, logL, stderror, tstat] = realized_egarch_midas_epu(daily_return, RV, EPU, sample_in_month, test_start, L)
%
% 2021/4/22
%   長期成分がEPUで駆動されるRealized EGARCH MIDASモデルの実装
%
% 各月の営業日の日数は,
...
%% 長期成分の推定
l = 1:L;
Gamma = beta_weight(l, L, gamma1, gamma2);

for t = L+1:T_long
    for ell = 1:L
        long_term(t,ell) = Gamma(ell) * EPU(t-ell);
    end
    g(t) = omega + (lambda^2) * sum(long_term(t,:));
end
...
%% Beta weight
function phi_ell = beta_weight(k, K, gamma1, gamma2)
% MIDAS項のbeta weight
% omega > 1である必要がある
j = 1:K;
