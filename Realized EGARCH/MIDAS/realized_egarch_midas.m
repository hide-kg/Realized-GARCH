function [estimpara, forecast_fit, logL, stderror, tstat] = realized_egarch_midas(daily_return, RV, test_start, K)
% 
% 2021/4/13
...
ver = 0;
llh = @(x0) -realized_egarch_midas_llh(x0, daily_return(1:test_start), RV(1:test_start), test_start, ver, K);
...
ver = 1;
[llh, ~, sigma, z, h, g] = realized_egarch_midas_llh(para, daily_return, RV, test_start, ver, K);

ver = 0;
fun = @(x0) realized_egarch_midas_llh(x0, daily_return(1:test_start), RV(1:test_start), test_start, ver, K);
VCV = vcv(fun, para);
...
function [llh, llhs, sigma, z, h, g] = realized_egarch_midas_llh(para0, daily_return, RV, test_start, ver, K)
%
...
for t = K+2:T
    ...
    % 長期成分の計算
    for k = 1:K
        long_term(t-1,k) = Gamma(k) * logx(t-k-1);
    end
